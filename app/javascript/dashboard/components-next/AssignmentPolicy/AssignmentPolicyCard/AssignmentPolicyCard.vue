      ...
      <div class="flex items-center gap-3 justify-between w-full">
        ...
        <div class="flex items-center gap-2">
          <Button
            ...
            @click="handleEdit"
          />
          <div v-if="order" class="w-px h-2.5 bg-accent" />
          <Button icon="i-lucide-trash" sm slate ghost @click="handleDelete" />
        </div>
      </div>
      <p class="text-muted-foreground text-body-para line-clamp-1 mb-0 py-1">
        {{ description }}
      </p>
      <div class="flex items-center gap-3 py-1.5">
        <span v-if="order" class="text-muted-foreground text-body-para">
          {{
            `${t('ASSIGNMENT_POLICY.AGENT_ASSIGNMENT_POLICY.INDEX.CARD.ORDER')}:`
          }}
          <span class="text-foreground">{{ order }}</span>
        </span>
        <div v-if="order" class="w-px h-3 bg-border" />
        <span v-if="priority" class="text-muted-foreground text-body-para">
          {{
            `${t('ASSIGNMENT_POLICY.AGENT_ASSIGNMENT_POLICY.INDEX.CARD.PRIORITY')}:`
          }}
          <span class="text-foreground">{{ priority }}</span>
        </span>
      </div>
    </div>
  </CardLayout>

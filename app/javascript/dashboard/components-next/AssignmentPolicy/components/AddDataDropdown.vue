...
<template>
  ...
  >
    ...
    <div
      v-if="showPopover"
      ref="dropdownRef"
      class="z-50 flex flex-col items-start absolute bg-accent backdrop-blur-[50px] border-0 gap-4 outline outline-1 outline-border rounded-xl max-w-96 min-w-80 max-h-[20rem] overflow-y-auto py-2"
      :class="[
        shouldShowAbove ? 'bottom-full mb-2' : 'top-full mt-2',
        shouldAlignRight ? 'right-0' : 'left-0',
      ]"
    >
      <div class="flex flex-col divide-y divide-muted w-full">
        <Input
          v-model="searchValue"
        ...
        >
          <template #prefix>
            <Icon
              ...
              class="absolute -translate-y-1/2 text-muted-foreground size-4 top-1/2 ltr:left-3 rtl:right-3"
            />
          </template>
        </Input>
        ...
        <div
          ...
          :key="item.id"
          class="flex gap-3 min-w-0 w-full py-4 px-3 hover:bg-accent cursor-pointer"
        ...
        >
          <Icon
            v-if="item.icon"
            :icon="item.icon"
            class="size-4 text-foreground flex-shrink-0 mt-0.5"
          />
          <span
          ...
          <div class="flex flex-col items-start gap-2 min-w-0 flex-1">
            <div class="flex items-center gap-1 min-w-0 w-full">
              <span
                ...
                class="text-sm text-foreground truncate min-w-0 flex-1"
              >
            ...
            <span
              ...
              class="text-sm text-muted-foreground truncate min-w-0 w-full block"
            >

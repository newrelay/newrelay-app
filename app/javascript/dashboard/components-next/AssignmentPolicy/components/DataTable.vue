  ...
  <div v-else class="flex flex-col divide-y divide-border">
    ...
    >
      ...
      >
        ...
        />
        <span
          class="text-sm text-foreground truncate min-w-0 group-hover:text-primary dark:group-hover:text-primary transition-colors"
        >
          {{ item.name }}
        </span>
        <Icon
          icon="i-lucide-external-link"
          class="size-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
        />
      </button>

...
<template>
  <div class="relative">
    <CardLayout
      ...
      layout="row"
      :class="{
        'outline-border !bg-muted dark:!bg-accent': isSelected,
      }"
    >
      <div class="flex items-center justify-start flex-1 gap-4">
        ...
        <div class="flex flex-col gap-0.5 flex-1">
          <div class="flex flex-wrap items-center gap-x-4 gap-y-1">
            ...
            <span class="inline-flex items-center gap-1">
              <span
                ...
                class="i-ph-building-light size-4 text-muted-foreground mb-0.5"
              />
              <span
                v-if="additionalAttributes?.companyName"
          ...
          >
            ...
            <div v-if="email" class="w-px h-3 truncate bg-border" />
            <span
              v-if="phoneNumber"
              class="text-sm truncate text-muted-foreground"
            >
            ...
            <div v-if="phoneNumber" class="w-px h-3 truncate bg-border" />
            <span
              v-if="countryDetails"
              class="inline-flex items-center gap-2 text-sm truncate text-muted-foreground"
            >
              <Flag :country="countryDetails.countryCode" class="size-3.5" />
              {{ formattedLocation }}
            </span>
            <div v-if="countryDetails" class="w-px h-3 truncate bg-border" />
            <Button

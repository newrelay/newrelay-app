    ...
    >
      <div class="flex flex-col gap-2">
        <button
          ...
          :key="inbox.id"
          type="button"
          class="flex items-center justify-between w-full px-4 py-2 text-left rounded-lg hover:bg-accent"
          @click="onPickInbox(inbox)"
        >
          <div class="flex items-center gap-2">
            <span class="i-ri-phone-fill text-muted-foreground" />
            <span class="text-sm text-foreground">{{ inbox.name }}</span>
          </div>
          <span v-if="inbox.phone_number" class="text-xs text-muted-foreground">
            {{ inbox.phone_number }}
          </span>
        </button>
      </div>

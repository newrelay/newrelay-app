...
const inputBorderClass = computed(() => {
  const errorClass =
    'outline-destructive dark:outline-destructive hover:outline-destructive dark:hover:outline-destructive disabled:outline-destructive dark:disabled:outline-destructive';
  const focusClass =
    'has-[:focus]:outline-primary dark:has-[:focus]:outline-primary';
  ...
  return `${focusClass} outline-border dark:outline-border hover:outline-border dark:hover:outline-border disabled:outline-border dark:disabled:outline-border`;
});
...
<template>
  <div>
    <div
      ...
      class="relative flex items-center h-8 transition-all duration-500 ease-in-out outline outline-1 outline-offset-[-1px] rounded-lg bg-black/10"
      ...
      >
        <template #prefix>
          <div class="flex items-center flex-shrink-0">
            ...
            <span
              v-if="activeCountry"
              class="text-sm left-[38px] top-2.5 text-muted-foreground ltr:!pl-1 rtl:!pr-1"
            >
              {{ activeDialCode }}
            </span>
          </div>
    ...
    <template v-if="phoneNumberError">
      <p
        ...
        class="min-w-0 mt-1 mb-0 text-xs truncate transition-all duration-500 ease-in-out text-destructive dark:text-destructive"
      >
        {{ phoneNumberError }}
      </p>
    </template>

...
Reference project: `/Users/deependrasankhala/Downloads/NewRelay-UI-main`
...
| `deals/DealsView.vue` | `routes/dashboard/deals/pages/DealsIndex.vue` + `components-next/Deals/` |
| `pipelines/PipelinesView.vue` | `routes/dashboard/deals/pages/PipelinesIndex.vue` (kanban of the same Deal rows) |
| `opportunities/OpportunitiesView.vue` | `routes/dashboard/deals/pages/OpportunitiesIndex.vue` (table of the same Deal rows) |
...
| `analytics/DashboardsView`, `RevenueAnalyticsView`, `CampaignAnalyticsView`; `tasks/`, `calendar/`, `ecommerce/*`; `reputation/`, `reviews/`, `funnels/`, `project-management/`, `surveys/`, `forms/`, `listings/` | **not built in Chatwoot** — sidebar shows the analytics ones disabled "Coming soon"; the rest have no Chatwoot equivalent |

...
<template>
  <button
    class="size-8 rounded-lg hover:bg-accent flex-shrink-0 grid place-content-center relative"
    @click="openNotificationPanel"
  >
    <span class="i-lucide-bell size-4" />
    <span
      v-if="unreadCount"
      class="min-h-2 min-w-2 p-0.5 px-1 bg-destructive rounded-lg absolute -top-1 -right-1.5 grid place-items-center text-[9px] leading-none text-destructive/10"
    >
      {{ unreadCount }}
    </span>
  </button>

...
<template>
  <div class="relative flex flex-col gap-1.5">
    ...
    <div
      class="flex flex-col gap-2 px-3 pt-3 pb-3 transition-all duration-500 ease-in-out border rounded-lg bg-black/10"
      :class="[
        customTextAreaWrapperClass,
        {
          'cursor-not-allowed opacity-50 !bg-black/10 disabled:border-border dark:disabled:border-border':
            disabled,
    ...
    >
      ...
      <textarea
        ...
        :disabled="disabled"
        rows="1"
        class="flex w-full reset-base text-sm p-0 !rounded-none !bg-transparent dark:!bg-transparent !border-0 !outline-0 !mb-0 placeholder:text-muted-foreground dark:placeholder:text-muted-foreground text-foreground dark:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
        @input="handleInput"
        @focus="handleFocus"
        @blur="handleBlur"
      />

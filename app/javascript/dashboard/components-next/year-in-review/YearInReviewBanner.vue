...
<template>
  <div v-if="shouldShowBanner" class="relative">
    <div
      class="mx-2 my-1 p-3 bg-primary rounded-lg cursor-pointer hover:shadow-md transition-all"
      @click="openModal"
    >
      <div class="flex items-start justify-between gap-2 mb-3">
        ...
        >
          <Icon
            icon="i-lucide-x size-4 mt-0.5 text-background dark:text-foreground"
          />
        </button>
      </div>
      ...
      <div class="flex flex-col gap-3">
        ...
        />
        <button
          class="w-full px-3 py-2 bg-white text-primary text-xs font-medium rounded-mdtracking-tight"

...
<template>
  <transition-group tag="div">
    <div
      v-for="step in steps"
      :key="step.route"
      class="cursor-pointer flex items-start gap-6 relative after:content-[''] after:absolute after:w-0.5 after:h-full after:top-5 ltr:after:left-4 rtl:after:right-4 before:content-[''] before:absolute before:w-0.5 before:h-4 before:top-0 before:left-4 rtl:before:right-4 last:after:hidden last:before:hidden after:bg-muted before:bg-muted"
    >
      <!-- Circle -->
      <div
        class="rounded-2xl flex-shrink-0 size-8 border-2 border-muted flex items-center justify-center left-2 leading-4 z-10 top-5 transition-all duration-300 ease-in-out"
        :class="{
          'bg-muted': step.isActive || step.isOver,
          'bg-background': !step.isActive && !step.isOver,
        }"
      >
        <span
          v-if="!step.isOver"
          ...
          :class="step.isActive ? 'text-primary' : 'text-muted-foreground'"
        >
          {{ step.index + 1 }}
        </span>
        <Icon

...
<template>
  <div>
    <ul
      v-if="items.length"
      ref="tagAgentsRef"
      class="vertical dropdown menu mention--box bg-card p-1 rounded-xl text-sm overflow-auto absolute w-full z-20 shadow-md left-0 leading-[1.2] bottom-full max-h-[12.5rem] border border-solid border-border"
      role="listbox"
    >
      <li
        v-for="item in items"
      ...
      >
        ...
        <div
          ...
          class="px-2 py-2 text-xs font-medium tracking-wide capitalize text-muted-foreground"
        >
          {{ item.title }}
        </div>
        <!-- Selectable Item -->
        <div
          v-else
          :class="{
            'bg-black/10': getSelectableIndex(item) === selectedIndex,
          }"
        ...
        >
          ...
          >
            <h5
              class="overflow-hidden mb-0 text-sm capitalize whitespace-nowrap text-muted-foreground text-ellipsis"
              :class="{
                'text-foreground': getSelectableIndex(item) === selectedIndex,
              }"
            >
              {{ item.displayName }}
            </h5>
            <div
              class="overflow-hidden text-xs whitespace-nowrap text-ellipsis text-muted-foreground"
              :class="{

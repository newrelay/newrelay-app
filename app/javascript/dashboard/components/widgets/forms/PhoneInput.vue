...
<template>
  <div class="relative phone-input--wrap">
    <div
      class="flex items-center justify-start border-none outline outline-1 rounded-lg bg-black/10"
      :class="
        error
          ? 'outline-destructive dark:outline-destructive hover:outline-destructive dark:hover:outline-destructive mb-1'
          : 'mb-4 outline-border dark:outline-border hover:outline-border dark:hover:outline-border'
      "
    >
      <div
        class="cursor-pointer py-2 pr-1.5 pl-2 rounded-tl-lg rounded-bl-lg flex items-center justify-center gap-1.5 bg-accent h-10 w-14"
      ...
      <span
        v-if="activeDialCode"
        class="flex py-2 ltr:pl-2 rtl:pr-2 text-base font-normal leading-normal text-foreground"
      >
        {{ activeDialCode }}
      </span>
      <input
    ...
    <div
      ...
      tabindex="0"
      class="z-10 absolute h-60 w-[12.5rem] shadow-md overflow-y-auto top-10 rounded-lg px-0 pt-0 pb-1 bg-accent backdrop-blur-[100px]"

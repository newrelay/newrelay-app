...
<template>
  <RelayModal
    :show="show"
    size="xl"
    :title="$t('SIDEBAR_ITEMS.KEYBOARD_SHORTCUTS')"
    @close="handleClose"
  >
    <div class="grid grid-cols-1 gap-x-16 gap-y-0 xl:grid-cols-2">
      <div
        class="flex items-center justify-between border-b border-border/40 py-3.5"
      >
      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <template v-for="(key, index) in shortcut.displayKeys" :key="index">
            ...
            <span v-else class="text-[13px] font-medium text-muted-foreground">
              {{ key }}
            </span>
          </template>
        </div>

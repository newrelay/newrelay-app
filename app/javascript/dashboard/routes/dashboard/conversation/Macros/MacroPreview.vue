  ...
  >
    ...
    >
      <div
        ...
        class="top-[0.390625rem] absolute -bottom-1 left-0 w-px bg-border"
      />
      <div
        class="absolute -left-[0.21875rem] top-[0.2734375rem] w-2 h-2 rounded-full bg-card border-2 border-solid border-border dark:border-border"
      />
      <p class="mb-1 text-xs text-muted-foreground">
        {{ $t(`MACROS.ACTIONS.${action.actionName}`) }}
      </p>
      <p class="text-foreground text-sm">{{ action.actionValue }}</p>
    </div>
  </div>
</template>

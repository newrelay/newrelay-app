...
import { defaultSmsTemplates, defaultEmailTemplates, defaultWhatsAppTemplates, defaultVideoTemplates } from './data/outreachTemplates';
...
// Channel invite text carries the {{VideoLink}} to the browser recorder.
const smsMessage = ref('Hi {{FirstName}}! We would love your quick 30s video feedback for {{BusinessName}}. Tap here to record: {{VideoLink}}');
const emailSubject = ref('{{FirstName}}, will you share a quick 45s video review?');
const emailBody = ref('Hi {{FirstName}},\n\nWe would love to feature your story! Could you take 45 seconds to record a short video testimonial for {{BusinessName}}?\n\nTap below to start recording in your browser (no app download needed):\n{{VideoLink}}\n\nThank you so much!');
const whatsappHeader = ref('Hi {{FirstName}} 🎥');
const whatsappBody = ref('We value your experience with {{BusinessName}}! Would you share a quick video testimonial? Tap below to record directly from your phone:\n\n{{VideoLink}}');
const whatsappButton1 = ref('🎥 Record Video Review');
const whatsappButton2 = ref('💬 Ask a Question');
...
const videoQuestions = ref(defaultVideoTemplates[0].questions.join('\n'));
const videoButtonText = ref(defaultVideoTemplates[0].buttonText);
const videoMaxDuration = ref(defaultVideoTemplates[0].maxDuration);
...
const parsedVideoQuestions = computed(() =>
  videoQuestions.value.split('\n').map(q => q.trim()).filter(Boolean)
);
const videoMaxSeconds = computed(() => (videoMaxDuration.value.match(/\d+/) || ['60'])[0]);
...
const fillVars = text => (text || '')
  ...
  .replaceAll('{{ReviewLink}}', 'relay.to/v/apex-dental')
...
function enhanceWithAi() {
  ...
  setTimeout(() => {
    if (activeEditingChannel.value === 'SMS') {
      smsMessage.value = 'Hi {{FirstName}}! We loved having you at {{BusinessName}}. Could you share a quick 45s video review? 🎥 {{VideoLink}}';
    } else if (activeEditingChannel.value === 'Email') {
      emailSubject.value = '{{FirstName}}, your story could inspire others at {{BusinessName}} 🌟';
      ...
      whatsappBody.value = 'Thank you for choosing {{BusinessName}} today! Would you take 45 seconds to record a short video review?\n\n{{VideoLink}}';
    }
...
async function generateReport() {
  ...
  // Fire one video-request per selected contact email (backend is email-only for now).
  const emails = form.value.selectedCustomers
  ...
  try {
    ...
    // non-blocking — still show the success step for this demo surface
  } finally {
    currentStep.value = 5;
  }
}
  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
    ...
    <div class="relative w-full max-w-5xl max-h-[90vh] bg-card rounded-2xl shadow-2xl border border-border flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      ...
      <div v-if="currentStep < 5" class="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/30">
        <div>
          <h2 class="text-xl font-semibold text-foreground">Request Video Testimonials</h2>
          <div class="flex items-center gap-2 mt-1.5 text-sm">
            ...
            <ChevronRight class="size-3.5 text-muted-foreground/50" />
            <span :class="currentStep >= 3 ? 'text-primary font-medium' : 'text-muted-foreground'">3. Video Template</span>
            <ChevronRight class="size-3.5 text-muted-foreground/50" />
      ...
      <div class="flex-1 overflow-y-auto p-0 bg-card hide-scrollbar flex flex-col">
        ...
        <div v-if="currentStep === 3" class="flex-1 flex flex-col lg:flex-row animate-in slide-in-from-right-4 duration-300 min-h-[450px] overflow-hidden">
          ...
          <div class="flex-1 p-5 sm:p-6 space-y-5 overflow-y-auto">
            ...
            <div class="space-y-4">
              <div class="flex flex-col gap-1.5">
                ...
                <Input v-model="videoHeadline" class="h-9 text-[14px]" />
              </div>
              <div class="flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <label class="text-[13.5px] font-medium text-foreground">Prompt Message</label>
                  <button type="button" class="h-7 px-2.5 gap-1.5 bg-primary/5 text-primary border border-primary/20 hover:bg-primary/10 rounded-lg text-xs font-semibold inline-flex items-center cursor-pointer disabled:opacity-60" :disabled="isAiEnhancing" @click="enhanceWithAi"><Sparkles class="size-3.5" :class="isAiEnhancing ? 'animate-spin' : ''" />{{ isAiEnhancing ? 'Enhancing…' : 'Improve with Relay AI' }}</button>
                </div>
                <textarea v-model="videoPrompt" rows="3" class="w-full text-[13.5px] p-3.5 shadow-xs rounded-xl border border-border/80 bg-background resize-none focus:outline-none focus:ring-1 focus:ring-primary/30 leading-relaxed"></textarea>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                  <label class="text-[13.5px] font-medium text-foreground">Guiding Questions</label>
                  <textarea v-model="videoQuestions" rows="3" class="w-full text-[13px] p-3 shadow-xs rounded-xl border border-border/80 bg-background resize-none focus:outline-none focus:ring-1 focus:ring-primary/30 leading-relaxed"></textarea>
                </div>
                <div class="flex flex-col gap-1.5">
            ...
            <div class="pt-4 border-t border-border space-y-4">
              ...
              <template v-if="activeEditingChannel === 'Email'">
                <div class="flex flex-col gap-1.5"><label class="text-[13.5px] font-medium text-foreground">Email Subject Line</label><Input v-model="emailSubject" class="h-9 text-[14px]" /></div>
                <div class="flex flex-col gap-2"><label class="text-[13.5px] font-medium text-foreground">Email Body Copy</label><textarea v-model="emailBody" rows="5" class="w-full text-[13.5px] p-3.5 shadow-xs rounded-xl border border-border/80 bg-background resize-none focus:outline-none focus:ring-1 focus:ring-primary/30 leading-relaxed"></textarea></div>
              </template>
              <template v-else-if="activeEditingChannel === 'SMS'">
                <div class="flex flex-col gap-2"><div class="flex items-center gap-2"><label class="text-[13.5px] font-medium text-foreground">SMS Message</label><span class="text-[11.5px] text-muted-foreground font-mono">({{ smsMessage.length }} / 160 chars)</span></div><textarea v-model="smsMessage" rows="4" class="w-full text-[13.5px] p-3.5 shadow-xs rounded-xl border border-border/80 bg-background resize-none focus:outline-none focus:ring-1 focus:ring-primary/30 leading-relaxed"></textarea></div>
              </template>
              <template v-else>
                <div class="flex flex-col gap-1.5"><label class="text-[13.5px] font-medium text-foreground">WhatsApp Header</label><Input v-model="whatsappHeader" class="h-9 text-[14px]" /></div>
                <div class="flex flex-col gap-2"><label class="text-[13.5px] font-medium text-foreground">WhatsApp Body</label><textarea v-model="whatsappBody" rows="4" class="w-full text-[13.5px] p-3.5 shadow-xs rounded-xl border border-border/80 bg-background resize-none focus:outline-none focus:ring-1 focus:ring-primary/30 leading-relaxed"></textarea></div>
              </template>

              <div class="space-y-2">
                <p class="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Insert Variables</p>
                <div class="flex flex-wrap gap-1.5">
                  <Badge v-for="v in ['{{FirstName}}', '{{BusinessName}}', '{{VideoLink}}', '{{EmployeeName}}']" :key="v" class="font-mono text-[11px] cursor-pointer bg-muted hover:bg-primary/20 text-foreground" @click="insertVariable(v)">{{ v }}</Badge>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Preview Side -->
          <div class="w-full lg:w-[345px] bg-muted/40 border-t lg:border-t-0 lg:border-l border-border p-4 sm:p-5 flex flex-col items-center shrink-0 overflow-hidden relative">
            <div class="absolute -top-12 -right-12 size-48 rounded-full bg-primary/10 blur-3xl pointer-events-none"></div>
            <div class="flex items-center gap-1 bg-card/80 backdrop-blur-md p-1 rounded-xl mb-3 border border-border/80 shadow-xs z-20">
              <button type="button" class="px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all cursor-pointer" :class="previewMode === 'video_page' ? 'bg-primary text-primary-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'" @click="previewMode = 'video_page'">Video Page</button>
              <button type="button" class="px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all cursor-pointer" :class="previewMode === 'invite_message' ? 'bg-primary text-primary-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'" @click="previewMode = 'invite_message'">Invite Message</button>
            </div>
            <div v-if="previewMode === 'invite_message' && form.channels.length > 1" class="flex items-center gap-1 bg-card/80 backdrop-blur-md p-1 rounded-xl mb-3 border border-border/80 shadow-xs z-20">
            ...
            <div class="w-[260px] sm:w-[272px] h-[460px] bg-card rounded-[30px] shadow-2xl border border-border flex flex-col overflow-hidden relative z-10">
              <div class="pt-3 px-4 pb-1 flex items-center justify-between text-[10.5px] font-medium text-foreground/80 shrink-0 select-none"><span class="font-semibold text-foreground">9:41</span><div class="flex items-center gap-1 text-foreground/70"><Signal class="size-2.5" /><Wifi class="size-2.5" /><div class="w-4 h-2 rounded-[2px] border border-foreground/70 p-0.5 flex items-center"><div class="h-full w-2 bg-foreground/80 rounded-[1px]"></div></div></div></div>

              <!-- VIDEO PAGE -->
              <div v-if="previewMode === 'video_page'" class="flex-1 flex flex-col overflow-hidden">
                <div class="px-3.5 py-2 flex items-center justify-between border-b border-border/40 shrink-0 bg-muted/20"><div class="flex items-center gap-1.5 min-w-0"><div class="size-6 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-[9px] shrink-0">NR</div><span class="text-[11px] font-semibold text-foreground truncate">Apex Dental &middot; Video</span></div><div class="flex items-center gap-1 text-[10px] text-muted-foreground font-mono"><Clock class="size-3 text-primary" /> {{ videoMaxSeconds }}s</div></div>
                <div class="flex-1 overflow-y-auto p-3 space-y-2.5 hide-scrollbar bg-muted/30">
                  <div class="p-3 bg-card border border-border/60 rounded-2xl shadow-xs space-y-2.5">
                    <div class="text-[12px] font-bold text-foreground leading-tight">{{ formattedVideoHeadline }}</div>
                    <div class="text-[11px] leading-relaxed text-muted-foreground whitespace-pre-wrap">{{ formattedVideoPrompt }}</div>
                    <div class="p-2.5 bg-primary/5 border border-primary/15 rounded-xl space-y-1.5">
                      ...
                      <ul class="text-[10px] text-muted-foreground space-y-1 pl-1"><li v-for="(q, i) in parsedVideoQuestions" :key="i" class="flex items-start gap-1"><span class="text-primary font-bold shrink-0">&bull;</span><span>{{ q }}</span></li></ul>
                    </div>
                    <div class="relative h-24 rounded-xl bg-black border border-slate-800 overflow-hidden flex flex-col items-center justify-center text-center p-2">
                      ...
                      <span class="text-[9.5px] font-medium text-white/90 mt-1">Camera Ready</span>
                      <span class="text-[8.5px] text-white/60 font-mono">00:00 / {{ videoMaxSeconds }}s</span>
                    </div>
                    <div class="w-full py-2 rounded-xl bg-primary text-primary-foreground text-[10.5px] font-semibold flex items-center justify-center gap-1.5 shadow-xs"><Play class="size-3 fill-current" /><span>{{ videoButtonText }}</span></div>
                  </div>
                </div>
              </div>

              <!-- INVITE EMAIL -->
              <div v-else-if="previewChannel === 'Email'" class="flex-1 flex flex-col overflow-hidden">
                <div class="px-3.5 py-2 flex items-center justify-between border-b border-border/40 shrink-0 bg-muted/20"><div class="flex items-center gap-2"><ChevronLeft class="size-4 text-primary shrink-0" /><span class="text-xs font-semibold text-foreground">Inbox</span></div><div class="flex items-center gap-2 text-muted-foreground"><Archive class="size-3.5" /><Trash2 class="size-3.5" /><MoreVertical class="size-3.5" /></div></div>
                <div class="flex-1 overflow-y-auto p-3 space-y-2.5 hide-scrollbar bg-muted/30">
                  <div class="space-y-1 pb-2 border-b border-border/40"><div class="text-[12px] font-bold text-foreground leading-tight">{{ formattedEmailSubject }}</div><div class="flex items-center justify-between pt-1"><div class="flex items-center gap-2 min-w-0"><div class="size-6 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-[10px] shrink-0">NR</div><div class="min-w-0"><div class="text-[11px] font-semibold text-foreground truncate">New Relay</div><div class="text-[9px] text-muted-foreground truncate">to sarah.j@gmail.com</div></div></div><span class="text-[9px] text-muted-foreground shrink-0">9:41 AM</span></div></div>
                  <div class="p-3 bg-card border border-border/60 rounded-2xl shadow-xs space-y-3"><div class="text-[11.5px] leading-relaxed whitespace-pre-wrap text-foreground">{{ formattedEmailBody }}</div><div class="p-2.5 rounded-xl bg-primary/10 border border-primary/20 text-center space-y-2"><div class="text-[11px] font-semibold text-primary">Record your video</div><div class="w-full py-1.5 rounded-lg bg-primary text-primary-foreground text-[10.5px] font-semibold flex items-center justify-center gap-1 shadow-xs"><Video class="size-3" /><span>Record Video</span></div><div class="text-[9px] font-mono text-primary flex items-center justify-center gap-1"><span>relay.to/v/apex-dental</span><ExternalLink class="size-2 shrink-0" /></div></div></div>
                </div>
              </div>

              <!-- INVITE WHATSAPP -->
              <div v-else-if="previewChannel === 'WhatsApp'" class="flex-1 flex flex-col overflow-hidden">
                <div class="px-3 py-2 bg-emerald-600 dark:bg-emerald-700 text-white flex items-center justify-between shrink-0 shadow-xs"><div class="flex items-center gap-2 min-w-0"><ChevronLeft class="size-4 shrink-0" /><div class="size-7 rounded-full bg-white/20 text-white flex items-center justify-center font-bold text-[10.5px] shrink-0 border border-white/30">NR</div><div class="min-w-0"><div class="flex items-center gap-1"><span class="text-xs font-semibold truncate text-white">New Relay</span><CheckCircle2 class="size-3 text-emerald-200 shrink-0" /></div><span class="text-[9px] text-emerald-100 block truncate">Official Business Account</span></div></div><div class="flex items-center gap-2.5 text-white/90"><Video class="size-3.5" /><Phone class="size-3.5" /><MoreVertical class="size-3.5" /></div></div>
                <div class="flex-1 overflow-y-auto p-3 space-y-2.5 bg-muted/40 hide-scrollbar"><div class="flex justify-center"><span class="text-[9px] bg-card text-muted-foreground px-2 py-0.5 rounded-md shadow-xs font-medium">Today</span></div><div class="space-y-1.5 max-w-[94%]"><div class="bg-card text-foreground rounded-2xl rounded-tl-xs p-3 shadow-xs border border-border/40 text-xs space-y-2"><div class="text-[12px] font-semibold text-foreground">{{ formattedWhatsAppHeader }}</div><div class="text-[11.5px] leading-relaxed whitespace-pre-wrap text-foreground">{{ formattedWhatsAppBody }}</div><div class="flex items-center justify-end gap-1 text-[8.5px] text-muted-foreground"><span>9:41 AM</span><span class="text-primary font-bold">&check;&check;</span></div></div><div class="space-y-1"><div class="w-full py-1.5 rounded-xl bg-card border border-border/60 text-[10.5px] font-semibold text-emerald-600 dark:text-emerald-400 text-center shadow-xs">{{ whatsappButton1 }}</div><div class="w-full py-1.5 rounded-xl bg-card border border-border/60 text-[10.5px] font-semibold text-muted-foreground text-center shadow-xs">{{ whatsappButton2 }}</div></div></div></div>
                <div class="p-2 bg-card border-t border-border/40 flex items-center gap-1.5 shrink-0"><Smile class="size-4 text-muted-foreground shrink-0" /><div class="flex-1 h-6 bg-muted/60 rounded-full px-2.5 text-[9.5px] text-muted-foreground flex items-center truncate">Message</div><Paperclip class="size-3.5 text-muted-foreground shrink-0" /><div class="size-6 rounded-full bg-emerald-600 text-white flex items-center justify-center shrink-0 shadow-xs"><Mic class="size-2.5" /></div></div>
              </div>
              ...
              <div v-else class="flex-1 flex flex-col overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto p-3 space-y-2.5 bg-muted/30 hide-scrollbar"><div class="text-[9.5px] text-center text-muted-foreground font-medium">Text Message &middot; Today 9:41 AM</div><div class="space-y-1 max-w-[94%]"><div class="bg-primary text-primary-foreground rounded-2xl rounded-tl-xs p-3 shadow-xs text-xs space-y-2"><div class="text-[11.5px] leading-relaxed whitespace-pre-wrap">{{ formattedSmsBody }}</div><div class="p-2 bg-white/15 border border-white/20 rounded-xl space-y-1 text-primary-foreground"><div class="flex items-center gap-1 text-[10px] font-semibold"><Video class="size-2.5 shrink-0" /><span>Record Video Review</span></div><div class="text-[9px] font-mono opacity-90 flex items-center gap-1 truncate"><span>relay.to/v/apex-dental</span><ExternalLink class="size-2 shrink-0" /></div></div></div><div class="text-[8.5px] text-right text-muted-foreground px-1">Delivered</div></div></div>
                <div class="p-2 bg-card border-t border-border/40 flex items-center gap-1.5 shrink-0"><div class="size-5 rounded-full bg-muted flex items-center justify-center text-muted-foreground font-bold text-xs shrink-0">+</div><div class="flex-1 h-6 bg-muted/60 rounded-full px-2.5 text-[9.5px] text-muted-foreground flex items-center truncate">Text Message</div><div class="size-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center shrink-0 shadow-xs"><Send class="size-2.5" /></div></div>
              </div>
            </div>

            <div class="text-center mt-2.5"><span class="text-[11px] text-muted-foreground font-medium">{{ previewMode === 'video_page' ? 'Recording Screen' : previewChannel + ' Invite Preview' }}</span></div>
          </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
              ...
              <div class="flex flex-col gap-4">
                ...
                <div class="bg-card border border-border rounded-xl p-5 shadow-xs flex items-center gap-4">
                  ...
                  <div>
                    <div class="text-xs text-muted-foreground mb-0.5">Video Template · {{ videoMaxDuration }}</div>
                    <div class="text-sm font-medium text-foreground">
                      {{ currentVideoTemplateName }}
                    </div>
                  </div>
                </div>
                ...
                <div class="bg-card border border-border rounded-xl p-5 shadow-xs space-y-3">
                  <div class="flex items-center gap-2 text-sm font-medium text-foreground"><Sparkles class="size-4 text-primary" /> Relay AI Processing</div>
                  <label class="flex items-center justify-between text-[13px] text-foreground cursor-pointer"><span>Generate transcript &amp; captions</span><input type="checkbox" v-model="form.transcript" class="accent-primary size-4 cursor-pointer" /></label>
                  <label class="flex items-center justify-between text-[13px] text-foreground cursor-pointer"><span>Auto-summarize key topics</span><input type="checkbox" v-model="form.summarize" class="accent-primary size-4 cursor-pointer" /></label>
                  <label class="flex items-center justify-between text-[13px] text-foreground cursor-pointer"><span>Detect sentiment</span><input type="checkbox" v-model="form.sentiment" class="accent-primary size-4 cursor-pointer" /></label>
                </div>

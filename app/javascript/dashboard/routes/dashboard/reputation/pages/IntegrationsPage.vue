...
import {
  RelayInput as Input, RelaySwitch, RelayTextarea, RelayButton,
...
const viewMode = ref(props.embedded ? 'grid3' : 'grid4');
...
const gIcon = GOOGLE_ICON_SVG;

const integrations = ref(reputationPlatformCatalog());
...
<template>
  ...
  >
    <div
      v-if="isDemoLoaded"
      class="space-y-6"
      :class="embedded ? 'w-full' : 'max-w-7xl mx-auto'"
    >
      <!-- Header -->
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-border/60">
        ...
        <div class="flex items-center gap-3">
          ...
          <RelayButton
            variant="outline"
            class="h-9 gap-2 border border-border bg-background shadow-xs text-[13.5px] font-medium hover:border-transparent"
            :disabled="isSyncing"
            @click="handleSyncAll"
          >
      ...
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pt-1">
        <div class="flex items-center gap-2 flex-wrap">
          <div class="relative w-48 sm:w-56">
            ...
            <Input v-model="searchQuery" placeholder="Search platforms…" class="pl-9 h-9 text-[13.5px] rounded-lg bg-background border border-border shadow-xs focus-visible:ring-1 focus-visible:ring-primary/30" />
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger as-child>
              <button class="h-9 gap-1.5 rounded-lg text-[13.5px] font-normal bg-background border border-border shadow-xs px-3 hover:border-transparent inline-flex items-center cursor-pointer">
          ...
          <button v-if="hasActiveFilters" class="h-9 rounded-lg text-[13px] text-muted-foreground hover:text-foreground border border-border hover:border-transparent gap-1 px-2.5 inline-flex items-center cursor-pointer" @click="resetFilters">
            <RotateCcw class="size-3" /> Reset
          </button>
        </div>
        <div class="flex items-center gap-2 self-end lg:self-auto">
          <div class="flex items-center bg-card border border-border rounded-lg p-0.5 shadow-xs">
            <button
              v-if="embedded"
      ...
      <div
        ...
        "
      >
        <div v-for="item in filteredIntegrations" :key="item.id" class="bg-card rounded-2xl border border-border p-6 shadow-xs flex flex-col justify-between hover:border-border/80 hover:shadow-sm transition-all">
          <div>

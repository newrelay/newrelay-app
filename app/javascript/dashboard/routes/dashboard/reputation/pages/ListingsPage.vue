...
function mapListing(row) {
  ...
  return {
    ...
    lastSync: row.synced_at ? new Date(row.synced_at).toLocaleString() : '—',
...
const stats = computed(() => {
  ...
  const connected = new Set(
    list.flatMap(l => (l.platforms || []).filter(p => p.status === 'Connected').map(p => p.name))
  );
  const needAttention = list.filter(l =>
    !(l.platforms || []).length || (l.platforms || []).some(p => p.status !== 'Connected')
  ).length;
  return [
...
async function loadListings() {
  ...
  try {
    ...
    listings.value = (Array.isArray(data) ? data : []).map(mapListing);
  } catch (err) {
    listings.value = [];
  } finally {
    loading.value = false;
      ...
      <div class="px-8 py-6 border-b border-border bg-white dark:bg-card shrink-0">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          <div v-for="s in stats" :key="s.label" class="bg-card border border-border rounded-xl p-4 shadow-xs flex flex-col justify-between">
            ...
            <span class="text-xs text-muted-foreground font-medium">{{ s.label }}</span>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div class="p-6 flex flex-col flex-1">
              ...
              <div class="flex items-center justify-between mt-6">
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <span v-if="!listing.platforms.length" class="text-xs text-muted-foreground">None connected</span>
                  </div>
                </div>
              ...
              <div class="flex items-center justify-between mt-6 pt-4 border-t border-border">
                <div class="flex items-center gap-3">
                  <router-link
                    :to="{ name: 'reputation_listing_detail', params: { listingId: listing.id } }"

...
<template>
  <div class="p-6 max-w-7xl mx-auto space-y-8">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-20 space-y-4">
      ...
      <p class="text-sm font-medium text-muted-foreground dark:text-muted-foreground">Loading dashboard...</p>
    </div>

    <!-- Onboarding State if not connected -->
    <div v-else-if="!isConnected" class="grid lg:grid-cols-12 gap-8 items-stretch">
      <div class="lg:col-span-7 flex flex-col justify-between p-8 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-800 shadow-sm">
        <div>
          ...
          <h2 class="capitalize text-3xl font-extrabold tracking-tight text-foreground dark:text-white mt-4">
            Build and Showcase Dynamic Social Proof
          </h2>
          <p class="text-muted-foreground dark:text-muted-foreground mt-3 text-base leading-relaxed">
          ...
          <div class="mt-8 space-y-4">
            <div class="flex gap-3">
              ...
              <div>
                <h4 class="capitalize font-bold text-foreground dark:text-slate-200 text-sm">Boost Search Rankings</h4>
                <p class="text-xs text-muted-foreground dark:text-muted-foreground">Drive active keyword review streams directly to your local listings.</p>
              </div>
            </div>
            <div class="flex gap-3">
              <div class="shrink-0 p-2 rounded-lg bg-primary/10 dark:bg-indigo-950/30 text-primary dark:text-indigo-400 h-9 w-9 flex items-center justify-center">
              ...
              <div>
                <h4 class="capitalize font-bold text-foreground dark:text-slate-200 text-sm">Draft Replies in Seconds</h4>
                <p class="text-xs text-muted-foreground dark:text-muted-foreground">Generate professional, context-aware responses with one click using AI.</p>
              </div>
            </div>
            <div class="flex gap-3">
              ...
              <div>
                <h4 class="capitalize font-bold text-foreground dark:text-slate-200 text-sm">Private Interceptor funnel</h4>
                <p class="text-xs text-muted-foreground dark:text-muted-foreground">Intercept negative ratings (1-3 stars) privately to fix issues before they go public.</p>
              </div>
            </div>
          </div>
        </div>

        <p class="text-xs text-muted-foreground mt-8 border-t border-slate-100 dark:border-slate-850 pt-4">
          By connecting your integrations, you authorize NewRelay to pull public review streams.
        </p>
      </div>

      <div class="lg:col-span-5 flex flex-col justify-between p-8 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-800 shadow-sm space-y-6">
        <div>
          <h3 class="capitalize text-xl font-bold text-foreground dark:text-white">Connect Review Platforms</h3>
          <p class="text-xs text-muted-foreground mt-1">Start syncing customer feedback instantly</p>
        </div>

        <div class="space-y-4">
          <!-- Google Profile connection -->
          <div class="p-4 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-border dark:hover:border-slate-700 transition-colors flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
              <div class="p-2 rounded-lg bg-red-50 dark:bg-red-950/20 text-destructive">
              ...
              <div>
                <h4 class="capitalize font-bold text-foreground dark:text-slate-200 text-sm">Google Maps Listing</h4>
                <p class="text-xs text-muted-foreground">Sync Google Business Profile reviews</p>
              </div>
            </div>
            <button
          ...
          <div class="p-4 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-border dark:hover:border-slate-700 transition-colors flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
              <div class="p-2 rounded-lg bg-primary/10 dark:bg-blue-950/20 text-primary">
              ...
              <div>
                <h4 class="capitalize font-bold text-foreground dark:text-slate-200 text-sm">Facebook Page</h4>
                <p class="text-xs text-muted-foreground">Sync ratings & page visitor feedback</p>
              </div>
            </div>
            <button
              disabled
              class="px-4 py-2 rounded-lg bg-muted dark:bg-slate-800 text-muted-foreground dark:text-muted-foreground text-xs font-semibold shadow-sm cursor-not-allowed"
            >
              Coming Soon
            </button>
          </div>
        ...
        <div class="p-4 rounded-xl bg-background dark:bg-slate-800/40 text-xs text-muted-foreground dark:text-muted-foreground leading-relaxed">
    ...
    <div v-else class="space-y-8 animate-fade">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 class="capitalize text-2xl font-extrabold text-foreground dark:text-white tracking-tight">Overview Dashboard</h2>
          <p class="text-xs text-muted-foreground mt-0.5">Real-time health index of your public ratings and responses</p>
        </div>
        <div class="flex gap-2">
          <button
            class="px-3.5 py-2 rounded-xl bg-white dark:bg-slate-850 hover:bg-background dark:hover:bg-slate-800 text-xs font-bold shadow-sm border border-border dark:border-slate-700 transition-all flex items-center gap-1.5"
            @click="loadData"
          >
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm relative overflow-hidden group">
          <p class="text-xs font-bold text-muted-foreground uppercase tracking-wider">Average Rating</p>
          <div class="flex items-baseline gap-2 mt-2">
            <span class="text-3xl font-extrabold text-foreground dark:text-white">{{ averageRating }}</span>
        ...
        <div class="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm">
          <p class="text-xs font-bold text-muted-foreground uppercase tracking-wider">Total Reviews</p>
          <div class="flex items-baseline gap-2 mt-2">
            <span class="text-3xl font-extrabold text-foreground dark:text-white">{{ totalReviewsCount }}</span>
            <span class="text-xs font-semibold text-muted-foreground">Synced reviews</span>
          </div>
          <p class="text-xs text-muted-foreground mt-2.5 flex items-center gap-1">
        ...
        <div class="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm">
          <p class="text-xs font-bold text-muted-foreground uppercase tracking-wider">AI Response Rate</p>
          <div class="flex items-baseline gap-2 mt-2">
            <span class="text-3xl font-extrabold text-foreground dark:text-white">{{ responseRate }}%</span>
            <span class="text-xs font-semibold text-woot-500">Replied</span>
          </div>
          <!-- progress bar -->
          <div class="w-full bg-muted dark:bg-slate-800 h-1.5 rounded-full mt-3.5 overflow-hidden">
            <div class="bg-woot-500 h-full rounded-full transition-all duration-300" :style="{ width: `${responseRate}%` }"></div>
          </div>
        </div>

        <div class="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm">
          <p class="text-xs font-bold text-muted-foreground uppercase tracking-wider">Pending Action</p>
          <div class="flex items-baseline gap-2 mt-2">
            <span class="text-3xl font-extrabold text-foreground dark:text-white">{{ pendingRepliesCount }}</span>
            <span class="text-xs font-semibold text-amber-500">Needs Reply</span>
          </div>
          <p class="text-xs text-muted-foreground mt-2.5">
            Avg reply time: <strong class="text-muted-foreground dark:text-slate-350">~15m</strong>
          </p>
        </div>
      </div>
      ...
      <div class="grid lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-7 p-6 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm space-y-4">
          <div>
            <h3 class="capitalize font-extrabold text-slate-950 dark:text-white text-base">Ratings Distribution</h3>
            <p class="text-xs text-muted-foreground">Score segmentation across synced listings</p>
          </div>

          <div class="space-y-3.5">
            <div v-for="stars in [5, 4, 3, 2, 1]" :key="stars" class="flex items-center gap-3">
              <span class="w-8 text-xs font-bold text-muted-foreground flex items-center justify-end gap-1">
                {{ stars }}<span class="text-yellow-400">★</span>
              </span>
              <div class="flex-1 bg-muted dark:bg-slate-800 h-2.5 rounded-full overflow-hidden">
                <div 
              ...
              <span class="w-10 text-xs font-bold text-muted-foreground text-right">
        ...
        <div class="lg:col-span-5 p-6 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm space-y-5">
          <div>
            <h3 class="capitalize font-extrabold text-slate-950 dark:text-white text-base">Active Connections</h3>
            <p class="text-xs text-muted-foreground">Synching locations and active listings</p>
          </div>
          ...
          <div class="space-y-3">
            <div
              ...
              :key="int.id"
              class="p-4 rounded-xl bg-background dark:bg-slate-850 border border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4"
            >
              <div class="flex items-center gap-3 min-w-0">
                <span 
                  v-if="int.provider === 'google'"
                  class="p-2 rounded-lg bg-red-100/50 dark:bg-red-950/20 text-destructive shrink-0"
                >
                ...
                <span 
                  ...
                  class="p-2 rounded-lg bg-blue-100/50 dark:bg-blue-950/20 text-primary shrink-0"
                >
                  <svg class="size-5" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </span>
                <div class="min-w-0">
                  <h4 class="font-bold text-foreground dark:text-slate-200 text-sm truncate capitalize">
                    {{ int.provider }}
                  </h4>
                  <p class="text-xs text-muted-foreground truncate mt-0.5 font-medium">
              ...
              <button
                class="text-xs text-destructive dark:text-red-400 font-bold hover:underline shrink-0 flex items-center gap-1"
            ...
            <div v-if="integrations.length < 2" class="flex gap-2 pt-2">
              <button
                ...
                class="flex-1 py-2 px-3 border border-border dark:border-slate-800 hover:bg-background dark:hover:bg-slate-800/50 rounded-xl text-xs font-bold text-muted-foreground dark:text-slate-350 transition-colors flex items-center justify-center gap-1.5"
                @click="connectGoogle"
              >
                Connect Google
              </button>
              <button
                ...
                disabled
                class="flex-1 py-2 px-3 border border-border dark:border-slate-800 bg-background dark:bg-slate-800/20 rounded-xl text-xs font-bold text-muted-foreground dark:text-muted-foreground cursor-not-allowed flex items-center justify-center gap-1.5"
              >
                Facebook (Soon)
              </button>
            </div>
      ...
      <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-border/80 dark:border-slate-850 shadow-sm space-y-5">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="capitalize font-extrabold text-slate-950 dark:text-white text-base">Reviews Requiring Attention</h3>
            <p class="text-xs text-muted-foreground">Quickly resolve recent feedback without leaving the dashboard</p>
          </div>
          <router-link
        ...
        <div v-if="recentPendingReviews.length === 0" class="flex flex-col items-center justify-center py-10 space-y-2">
          ...
          <p class="text-sm font-bold text-foreground dark:text-slate-300">All caught up!</p>
          <p class="text-xs text-muted-foreground">Every synced review has been responded to or ignored.</p>
        </div>
        ...
        <div v-else class="space-y-4">
          <div
            ...
            :key="review.id"
            class="p-4 rounded-xl border border-slate-100 dark:border-slate-850 hover:bg-background/50 dark:hover:bg-slate-850/40 transition-colors"
          >
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div class="space-y-1">
                <div class="flex items-center gap-2 flex-wrap">
                  <span class="text-xs font-bold text-foreground dark:text-slate-300">{{ review.reviewer_name }}</span>
                  <span class="size-1 bg-slate-300 rounded-full"></span>
                  <span class="text-yellow-400 font-bold text-xs">
                    {{ '★'.repeat(review.rating) }}{{ '☆'.repeat(5 - review.rating) }}
                  </span>
                  <span class="size-1 bg-slate-300 rounded-full"></span>
                  <span class="text-[10px] uppercase font-bold text-muted-foreground px-2 py-0.5 bg-muted dark:bg-slate-800 rounded-full capitalize">
                    {{ review.provider }}
                  </span>
                </div>
              ...
              <div class="flex gap-2 self-start shrink-0">
                <button
                  class="px-3 py-1.5 text-xs font-bold text-muted-foreground dark:text-slate-300 border border-border dark:border-slate-700 hover:bg-background dark:hover:bg-slate-800 rounded-lg transition-colors"
            ...
            <div v-if="activeReview?.id === review.id" class="mt-4 border-t border-slate-100 dark:border-slate-800 pt-4 space-y-3">
              <label class="block text-muted-foreground uppercase tracking-wider text-[13.5px] font-[500] text-foreground">Compose Reply</label>
              <textarea
                v-model="replyBody"
                rows="3"
                class="w-full rounded-xl border border-border dark:border-slate-700 dark:bg-slate-800 dark:text-slate-250 p-3.5 focus:outline-none focus:ring-2 focus:ring-woot-500 text-[14px] border-border/80 bg-background focus-visible:ring-1 focus-visible:ring-primary/30 shadow-sm rounded-md"
                :placeholder="drafting ? 'Drafting response via AI assistant...' : 'Write response here...'"
              />
              <div class="flex justify-between items-center">
                <span class="text-[10px] text-muted-foreground">AI drafted response can be fully edited before publishing.</span>
                <div class="flex gap-2">
                  <button
                    class="px-3 py-1.5 text-xs font-bold text-muted-foreground hover:bg-background dark:hover:bg-slate-800 rounded-lg border border-slate-150 dark:border-slate-850"

  ...
  <div class="flex-1 overflow-y-auto w-full hide-scrollbar bg-background p-6 lg:p-8">
    ...
    <div class="max-w-7xl mx-auto space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <!-- AI Sentiment Card (Vibrant Indigo/Primary Card matching Reference Design 1:1) -->
        <div class="bg-primary text-primary-foreground rounded-2xl border border-transparent shadow-xs p-5 relative overflow-hidden group hover:shadow-md transition-all flex flex-col justify-between">
          <!-- Decorative blurred ambient background circle -->
          <div class="absolute top-0 right-0 -mt-4 -mr-4 size-24 bg-white/10 rounded-full blur-xl pointer-events-none"></div>
          
          <div class="flex justify-between items-start mb-4 relative z-10">
            <div>
              <p class="text-[13px] font-medium text-primary-foreground/80 mb-1">AI Sentiment</p>
              <h3 class="text-3xl font-bold text-primary-foreground">92% <span class="text-lg font-medium text-primary-foreground/90 ml-1">Positive</span></h3>
            </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-card rounded-2xl border border-border shadow-xs p-6 lg:col-span-2 flex flex-col justify-between">
          <div class="flex justify-between items-center mb-6">
            ...
            <div class="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground shrink-0 cursor-pointer hover:bg-muted">
              Last 7 Months
          ...
          <div class="h-[220px] w-full flex items-end justify-between gap-3 pt-6 px-2">
            <div v-for="(bar, idx) in [
              { month: 'Jan', val1: 65, val2: 35 },
              { month: 'Feb', val1: 59, val2: 30 },
              { month: 'Mar', val1: 80, val2: 45 },
              { month: 'Apr', val1: 81, val2: 50 },
              { month: 'May', val1: 56, val2: 25 },
              { month: 'Jun', val1: 55, val2: 28 },
              { month: 'Jul', val1: 72, val2: 40 }
            ]" :key="idx" class="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
        ...
        <div class="bg-card rounded-2xl border border-border shadow-xs p-0 flex flex-col overflow-hidden relative">
          ...
          <div class="p-6 flex-1 flex flex-col">
            ...
            <div class="space-y-4 flex-1">
              <div class="flex gap-3 items-start p-2 rounded-lg hover:bg-muted/40 transition-colors">
                <div class="mt-1 size-2 rounded-full bg-emerald-500 shrink-0"></div>
                <div>
          ...
          <div class="w-full">
            ...
            <div class="space-y-4">
              <div v-for="platform in platforms" :key="platform.name" class="grid grid-cols-12 gap-2 items-center text-sm py-1 border-b border-border/40 last:border-0">
                ...
                <div class="col-span-2 text-center text-muted-foreground font-medium text-xs">
                  {{ platform.total }}
                </div>
                ...
                <div class="col-span-3 flex justify-end">
                  ...
                  <svg v-if="platform.trend === 'down'" viewBox="0 0 40 15" class="w-10 h-4 stroke-rose-500 fill-none" stroke-width="2" stroke-linecap="round"><path d="M0,5 L10,7 L20,12 L30,10 L40,15"></path></svg>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <router-link :to="{ name: 'reputation_reviews' }" class="flex items-center gap-3 p-4 border border-border rounded-xl hover:bg-muted/50 transition-colors text-left group">
              ...
              <div>
                ...
                <p class="text-[11px] text-muted-foreground mt-0.5">3 pending items</p>
              </div>
            </router-link>
              ...
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                <div>
                  ...
                  <div class="flex items-center gap-2 mt-1">
                    <div class="flex gap-0.5 text-amber-400">
                      <Star v-for="i in 5" :key="i" class="size-3.5 fill-amber-400 text-amber-400" :class="i <= review.rating ? 'fill-amber-400' : 'text-muted-foreground/30'" />
                    </div>

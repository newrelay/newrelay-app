...
const STATUS_COLORS = {
  ...
  AVAILABLE_WITHOUT_REVIEW: 'text-success',
...
const getModeStatusTextColor = mode => MODE_COLORS[mode] || 'text-foreground';

const getStatusTextColor = status => STATUS_COLORS[status] || 'text-foreground';
...
<template>
  <div class="gap-4 mx-6">
    <div
      class="px-5 py-5 space-y-6 rounded-xl outline outline-1 -outline-offset-1 outline-border bg-secondary"
    >
      <div
      ...
      >
        <div>
          ...
          <p class="mt-1 text-body-main text-muted-foreground">
      ...
      <div v-if="healthData" class="grid grid-cols-1 gap-4 xs:grid-cols-2">
        <div
          ...
          :key="item.key"
          class="flex flex-col gap-2 p-4 rounded-lg border border-border bg-card"
        >
          ...
          <div class="flex items-center">
            <span
              ...
              class="inline-flex items-center px-2 py-0.5 min-h-6 text-label-small rounded-md bg-accent"
            ...
            <span v-else class="text-label text-foreground">{{
              item.value
            }}</span>
          </div>
        </div>
        ...
        >
          ...
          <div class="flex items-center justify-between gap-3">
            <span
              ...
              class="inline-flex items-center gap-1.5 px-2 py-0.5 min-h-6 text-label-small rounded-md bg-accent text-success"
            >
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 px-2 py-0.5 min-h-6 text-label-small rounded-md bg-accent text-warning"
            >
      ...
      <div v-else class="pt-8">
        <div
          class="flex justify-center items-center p-8 text-center text-muted-foreground"
        >
          <div>

        ...
        <div class="max-w-3xl space-y-8">
          <div class="h-full">
            <div class="h-full animate-in fade-in duration-300">
              <div class="border border-border/60 bg-card rounded-xl shadow-xs overflow-hidden mb-8">
                <!-- Header -->
                <div class="p-4 sm:p-6 border-b border-border/40">
                  ...
                  <p class="text-sm text-muted-foreground mt-1 max-w-4xl">
                    Service Level Agreements (SLAs) are contracts that define clear expectations between your team and customers. They establish standards for response and resolution times, creating a framework for accountability and ensures a consistent, high-quality experience.
                  </p>
                </div>
                ...
                <div class="p-4 sm:p-6 space-y-6">
                  ...
                  <div class="mt-8">
                    <!-- Empty State -->
                    <div v-if="emptyTableMessage" class="py-20 flex flex-col items-center justify-center text-center">
                    ...
                    <div v-else class="min-w-full overflow-x-auto">
                      <!-- Header row -->
                      <div class="grid grid-cols-[1.5fr_1fr_0.7fr_0.7fr_0.7fr_100px] items-center border-b border-border/40 px-4 py-3 bg-muted/20">
                        ...
                        <div class="flex items-center gap-1 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
                          {{ $t('SLA.LIST.RESPONSE_TYPES.SHORT_HAND.FRT') }}
                          <RelayTooltip :content="$t('SLA.LIST.RESPONSE_TYPES.FRT')" side="top">
                            <Icon icon="i-lucide-info" class="size-3.5 cursor-help" />
                          </RelayTooltip>
                        </div>
                        <div class="flex items-center gap-1 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
                          {{ $t('SLA.LIST.RESPONSE_TYPES.SHORT_HAND.NRT') }}
                          <RelayTooltip :content="$t('SLA.LIST.RESPONSE_TYPES.NRT')" side="top">
                            <Icon icon="i-lucide-info" class="size-3.5 cursor-help" />
                          </RelayTooltip>
                        </div>
                        <div class="flex items-center gap-1 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
                          {{ $t('SLA.LIST.RESPONSE_TYPES.SHORT_HAND.RT') }}
                          <RelayTooltip :content="$t('SLA.LIST.RESPONSE_TYPES.RT')" side="top">
                            <Icon icon="i-lucide-info" class="size-3.5 cursor-help" />
                          </RelayTooltip>
                        </div>
                        <div class="text-right text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
                        ...
                        >
                          ...
                          <div>
                            <RelayBadge
                              ...
                              :class="sla.only_during_business_hours ? 'border-primary/20 bg-primary/10 text-primary' : ''"
                            >
                              <Icon
                                :icon="sla.only_during_business_hours ? 'i-lucide-alarm-clock-check' : 'i-lucide-alarm-clock-off'"
                                class="size-3.5"
                              />
                              {{ sla.only_during_business_hours ? $t('SLA.LIST.BUSINESS_HOURS_ON') : $t('SLA.LIST.BUSINESS_HOURS_OFF') }}
                            </RelayBadge>
                          </div>
                          ...
                          <div class="flex items-center justify-end gap-1.5 opacity-0 transition-opacity group-hover:opacity-100">
                            <RelayTooltip :content="$t('SLA.FORM.EDIT')" side="top">
                              ...
                              >
                                <Icon icon="i-lucide-pencil" class="size-3.5 text-muted-foreground" />
                              </button>
                            </RelayTooltip>
                            <RelayTooltip :content="$t('SLA.FORM.DELETE')" side="top">
                              ...
                              >
                                <Icon icon="i-lucide-trash-2" class="size-3.5 text-muted-foreground hover:text-destructive" />
                              </button>
                            </RelayTooltip>
                          </div>
                        </div>

...
const platforms = [
  { name: 'Google Business Profile', positivePercent: 96, count: 452, rating: '4.8' },
  { name: 'Facebook Pages', positivePercent: 95, count: 312, rating: '4.9' },
  { name: 'Trustpilot Portal', positivePercent: 93, count: 204, rating: '4.6' },
  { name: 'Yelp for Business', positivePercent: 88, count: 128, rating: '4.3' }
]

const positiveDrivers = [
  { topic: 'Staff Friendliness & Professionalism', score: 98, mentions: 412 },
  { topic: 'Speed of Service & Support', score: 94, mentions: 326 },
  { topic: 'Quality of Care / Experience', score: 96, mentions: 284 },
  { topic: 'Transparent & Fair Pricing', score: 91, mentions: 198 }
]

const improvementTopics = [
  { topic: 'Phone & Check-in Wait Times', score: 14, mentions: 28 },
  { topic: 'Weekend Appointment Availability', score: 8, mentions: 16 }
]
...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
    ...
    <div class="relative w-full max-w-2xl max-h-[90vh] bg-card rounded-2xl shadow-2xl border border-border flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      ...
      <div class="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/30 shrink-0">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[12.5px] text-muted-foreground mt-0.5">Aggregated sentiment and customer topic analysis across all channels.</p>
          </div>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6 hide-scrollbar">
        ...
        <div class="grid grid-cols-3 gap-3">
          <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-center">
            ...
            <div class="text-[11px] text-muted-foreground mt-0.5">1,030 reviews</div>
          </div>
          ...
          <div class="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-center">
            ...
            <div class="text-[11px] text-muted-foreground mt-0.5">44 reviews</div>
          </div>
          ...
          <div class="p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-center">
            ...
            <div class="text-[11px] text-muted-foreground mt-0.5">22 reviews</div>
          </div>
        </div>
        ...
        <div class="space-y-1.5">
          <div class="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <span class="font-medium text-foreground">1,096 Total Customer Ratings</span>
          </div>
          <div class="h-3 w-full bg-muted rounded-full overflow-hidden flex gap-0.5 p-0.5">
            <div class="bg-emerald-500 rounded-l-full h-full" style="width: 94%" title="94% Positive"></div>
            <div class="bg-amber-400 h-full" style="width: 4%" title="4% Neutral"></div>
            <div class="bg-destructive rounded-r-full h-full" style="width: 2%" title="2% Negative"></div>
          </div>
        </div>
        ...
        <div class="space-y-3 pt-2 border-t border-border">
          ...
          <div class="space-y-2.5">
            <div 
              ...
              :key="platform.name"
              class="p-3 bg-muted/30 border border-border rounded-xl flex items-center justify-between"
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-muted-foreground">{{ platform.count }} verified reviews · {{ platform.rating }} ★ avg</div>
                </div>
              </div>
        ...
        <div class="space-y-3 pt-2 border-t border-border">
          ...
          <div class="space-y-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div 
                v-for="item in positiveDrivers" 
                :key="item.topic"
                class="p-2.5 rounded-lg bg-emerald-500/5 border border-emerald-500/15 flex items-center justify-between text-xs"
              >
          ...
          <div class="space-y-2 pt-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div 
                v-for="item in improvementTopics" 
                :key="item.topic"
                class="p-2.5 rounded-lg bg-amber-500/5 border border-amber-500/15 flex items-center justify-between text-xs"
              >
                <div class="flex items-center gap-2">

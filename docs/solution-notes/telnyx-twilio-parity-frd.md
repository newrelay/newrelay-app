...
- §6a (virtual number purchase, PRD-18) now has a Technical Implementation Spec: [telnyx-virtual-number-purchase-implementation-spec.md](telnyx-virtual-number-purchase-implementation-spec.md) — Draft, unapproved. Written directly from this FRD (no Architecture Design gate — see the spec's process note for why).
...
13. **New, with PRD-19: does this reseller sub-flow need its own Architecture Design before the Implementation Spec is finalized?** Flagged as a strong recommendation in PRD-19 above (shared platform credential, billing integration, tenant isolation, expensive-to-reverse) — but not yet actually decided/scheduled.
14. **🚨 Blocking — narrowed to a real product decision (2026-09-24): does India stay in Phase 1's SMS scope at all, given Telnyx has zero SMS-capable numbers there?** Confirmed two ways now — live API test, and Telnyx's own India Mobile Numbers release note (2026-09-17), which scopes the launch to voice-only. **The "maybe it's account-tier-specific" possibility is ruled out** — this is Telnyx's actual shipped product scope for India, not a limitation of the test account. Two real paths remain: (a) drop India from §6a's SMS scope entirely, keep Phase 1 SMS as US-only; (b) keep India, but as a **Voice-only** capability — which means it logically belongs under PRD-10/§6b's Voice work instead of §6a's number-purchase-for-SMS flow, a real re-scoping, not a copy-paste move. This is now a Product decision, not an engineering or an account-tier one — needs an answer before the Implementation Spec's India-specific SMS work (schema, KYC flow, district picker) is worth continuing, since it's currently built for a capability Telnyx's India numbers don't have.

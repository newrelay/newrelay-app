...
>
> **Scope note:** this document covers only PRD-10 (Twilio-parity enterprise Voice). It does **not** cover the FRD's §6a "AI Voice Agent" (Telnyx number + ElevenLabs Conversational AI via SIP trunking) — that's a separate, architecturally unrelated integration (ElevenLabs handles the call directly, NR is not in the call-control loop) that needs its own design if/when it's scoped.

...
RSpec.describe 'MCP add_private_note', type: :request do
  ...
  it 'adds a private note that is never customer-visible' do
    mcp_call('add_private_note', arguments: { account_id: account.id, conversation_id: conversation.display_id, content: 'internal note' },
                                 token: agent.access_token.token)

    expect(mcp_result['isError']).to be false
    body = mcp_content_json
    expect(body['content']).to eq('internal note')
  ...
  it 'returns the same not-found error for a real conversation the agent has no inbox/team access to' do
    ...
    mcp_call('add_private_note', arguments: { account_id: account.id, conversation_id: other_conversation.display_id, content: 'x' },
                                 token: agent.access_token.token)
  ...
  it 'returns a tool error for an unknown conversation_id' do
    mcp_call('add_private_note', arguments: { account_id: account.id, conversation_id: 999_999, content: 'x' },
                                 token: agent.access_token.token)
